...
import { IconSearch, IconAnnouncement, IconUsers, IconTrash, IconSend, IconX, IconCheck, IconShield, IconCheckCircle, IconCrown } from '../components/Icons';
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          <div className="grid grid-cols-4 gap-2">
            <GlassCard className="p-3">
              <p className="text-gray-500 text-[10px] font-body font-medium mb-0.5">总用户</p>
              <p className="text-white font-display font-700 text-xl">{stats.total}</p>
            </GlassCard>
            <GlassCard className="p-3">
              <p className="text-gray-500 text-[10px] font-body font-medium mb-0.5">在线</p>
              <div className="flex items-baseline gap-1">
                <p className="text-biu-primary font-display font-700 text-xl">{stats.online}</p>
                <span className="w-1.5 h-1.5 rounded-full bg-biu-primary animate-pulse-subtle" />
              </div>
            </GlassCard>
            <GlassCard className="p-3">
              <p className="text-gray-500 text-[10px] font-body font-medium mb-0.5">管理员</p>
              <p className="text-amber-400 font-display font-700 text-xl">{stats.admins}</p>
            </GlassCard>
            <GlassCard className="p-3">
              <p className="text-gray-500 text-[10px] font-body font-medium mb-0.5">认证</p>
              <p className="text-biu-primary font-display font-700 text-xl">{stats.verified}</p>
            </GlassCard>
          </div>
          ...
          <GlassCard className="p-4">
            <div className="flex items-center gap-2 mb-4">
              <div className="w-6 h-6 rounded-lg bg-biu-primary/15 flex items-center justify-center">
                <IconAnnouncement size={13} className="text-biu-primary" />
              </div>
            ...
            <div className="space-y-2.5">
              <input
                ...
                className="w-full px-3 py-2.5 rounded-xl glass-input text-white placeholder-gray-600 outline-none font-body text-sm"
                placeholder="广播标题"
              />
              <div className="relative">
                <textarea
                  ...
                  className="w-full px-3 py-2.5 rounded-xl glass-input text-white placeholder-gray-600 outline-none font-body text-sm min-h-[80px] resize-none"
                  placeholder="写下你想对所有用户说的话..."
                />
                <span className="absolute bottom-2.5 right-3 text-[11px] font-body text-gray-600">
              ...
              <div className="flex items-center justify-between pt-0.5">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-biu-primary hover:bg-biu-primary-dim text-biu-dark font-display font-600 text-sm transition-all duration-200 hover:shadow-glow disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  {loading ? (
          ...
          <GlassCard className="p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="px-1.5 py-0.5 rounded-md bg-white/8 text-gray-500 text-[10px] font-body font-medium">
            ...
            <div className="relative mb-3">
              <IconSearch size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-600" />
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="搜索用户名、昵称或 Biu 号"
                className="w-full pl-8 pr-4 py-2 rounded-xl glass-input text-white placeholder-gray-600 outline-none font-body text-sm"
              />
              {searchQuery && (
                <button
              ...
              {filteredUsers.length === 0 ? (
                ...
                filteredUsers.map((u) => {
                  ...
                  return (
                    <div
                      key={u.id}
                      className="group flex items-center gap-2.5 px-2.5 py-2 rounded-xl hover:bg-white/[0.04] transition-all duration-200"
                    >
                      ...
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                        <button
                          ...
                          className={`flex items-center gap-1 px-1.5 py-0.5 rounded-lg ${officialCfg.bg} ${officialCfg.color} text-[10px] font-body font-600 transition-all duration-200`}
                        >
                        ...
                        <button
                          ...
                          className={`flex items-center gap-1 px-2 py-1 rounded-lg ${roleCfg.bg} hover:bg-white/12 ${roleCfg.color} text-[10px] font-body font-600 transition-all duration-200`}
                        >
                          <IconShield size={9} />
                          {roleCfg.label}
                        </button>
                        ...
                        <button
                          ...
                          className="p-1 rounded-lg text-gray-600 hover:text-biu-accent hover:bg-biu-accent/10 transition-all duration-200"
                          title="删除用户"
                        >
                          <IconTrash size={13} />
                        </button>
